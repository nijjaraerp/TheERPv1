...
function setupAutoIncrementIDs() {
    ...
    try {
      ...
      if (idColumnIndex !== -1) {
        ...
        if (hasFormula(sheet, targetRow, targetCol)) {
          Logger.log(`Auto-increment formula already exists for ${sheetName} column ${headers[idColumnIndex]}, skipping`);
          skipCount++;
          return;
        }
        ...
        if (existingValue !== null && existingValue !== '' && !isNaN(existingValue)) {
          Logger.log(`Data exists in ${sheetName} row ${targetRow}, column ${headers[idColumnIndex]}, skipping to preserve data`);
          skipCount++;
          return;
        }
  ...
  Logger.log(`Auto-increment ID setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupDropdownValidations() {
    ...
    try {
      ...
      Object.keys(dropdownMappings[sheetName]).forEach(columnName => {
        try {
          ...
          if (existingValidation && existingValidation.getCriteriaType() !== SpreadsheetApp.DataValidationCriteria.UNCHECKED) {
            Logger.log(`Dropdown validation already exists for ${sheetName}.${columnName}, skipping`);
            skipCount++;
            return;
          }

          // Check if named range exists
          const named = SpreadsheetApp.getActiveSpreadsheet().getRangeByName(ddId);
          if (!named) {
            Logger.log(`Named range ${ddId} not found, skipping ${sheetName}.${columnName}`);
            skipCount++;
            return;
          }
          ...
          errorCount++;
          Logger.log(`Error setting up dropdown validation for ${sheetName}.${columnName}: ${error.toString()}`);
        }
      });
  ...
  Logger.log(`Dropdown validations setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupCalculatedColumns() {
    ...
    try {
      ...
      Object.keys(calculatedColumns[sheetName]).forEach(columnName => {
        try {
          ...
          // Check if formula already exists
          if (hasFormula(sheet, targetRow, targetCol)) {
            Logger.log(`Calculated formula already exists for ${sheetName}.${columnName}, skipping`);
            skipCount++;
            return;
          }
          ...
          errorCount++;
          Logger.log(`Error setting up calculated column ${columnName} in ${sheetName}: ${error.toString()}`);
        }
      });
  ...
  Logger.log(`Calculated columns setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupAuditColumns() {
    ...
    try {
      ...
      if (crtAtIndex !== -1) {
        ...
        if (!hasFormula(sheet, targetRow, targetCol)) {
          ...
          Logger.log(`Crt_At formula already exists for ${sheetName}, skipping`);
          skipCount++;
        }
      }
      ...
      if (crtByIndex !== -1) {
        ...
        Logger.log(`Crt_By column found in ${sheetName}, but requires application-level implementation`);
      }
  ...
  Logger.log(`Audit columns setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupViewColumns() {
      ...
      if (!headers) {
        ...
        if (deptNameIndex !== -1) {
          ...
          if (viewColumnIndex !== -1) {
            Logger.log(`View column ${viewColumnName} already exists in HRM_Employees, skipping`);
            skipCount++;
          } else {
            ...
            if (!hasData(empSheet, 1, newViewColumnIndex) && !hasData(empSheet, 2, newViewColumnIndex)) {
              // Add header for view column
              empSheet.getRange(1, newViewColumnIndex, 1, 1).setValue(viewColumnName);
              empSheet.getRange(2, newViewColumnIndex, 1, 1).setValue('اسم القسم');
              ...
              if (!hasFormula(empSheet, 3, newViewColumnIndex)) {
                empSheet.getRange(3, newViewColumnIndex, 1, 1).setFormula(formula);
                successCount++;
                Logger.log('Set up department name view column in HRM_Employees');
              } else {
                skipCount++;
                Logger.log('Formula already exists for view column in HRM_Employees, skipping');
              }
            } else {
              skipCount++;
              Logger.log(`Data exists in view column position for HRM_Employees, skipping to preserve data`);
            }
          }
        } else {
          skipCount++;
          Logger.log('DEPT_Name column not found in HRM_Employees, skipping view column setup');
        }
      }
    } else {
      skipCount++;
  ...
  Logger.log(`View columns setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupConditionalFormatting() {
    ...
    if (empSheet) {
      ...
      if (!headers) {
        ...
        if (statusIndex !== -1) {
          ...
          if (existingRules && existingRules.length > 0) {
            ...
            const hasStatusRules = existingRules.some(rule => {
              try {
                ...
                return formula && (formula.toString().includes('نشط') || formula.toString().includes('غير نشط'));
              } catch (e) {
                return false;
              }
            });
            ...
            if (hasStatusRules) {
              Logger.log('Conditional formatting rules already exist for employee status, skipping');
              skipCount++;
            } else {
              // Add new rules
          ...
          skipCount++;
          Logger.log('EMP_Status column not found in HRM_Employees, skipping conditional formatting');
        }
      }
    } else {
      skipCount++;
      Logger.log('HRM_Employees sheet not found, skipping conditional formatting');
    }
  ...
  Logger.log(`Conditional formatting setup completed: ${successCount} successful, ${skipCount} skipped, ${errorCount} errors`);
}
...
function setupAllFormulas() {
  ...
  try {
    ...
    return {
      success: true,
      results: results,
      message: 'All formulas and validations have been set up (bilingual headers are handled by Setup.js)'
    };
